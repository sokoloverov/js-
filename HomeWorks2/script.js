...
const goods = [
    ...
    { title: 'Знание js', price: 'бесценно', picture: 'js.jpg' },
 ...
 */
const renderGoodsItem = (title = 'Ожидаем...', price = 'кто знает?', picture = 'waiting.png') =>
    `<div class="goods_box">
        <a href="#">
            <figure>
                <img class="goods_box__img" src="img/${picture}" alt="Товар">
                <figcaption class="goods_box_upper_font">${title}</figcaption>
                <figcaption class="goods_box_lower_font">${price}</figcaption>
            </figure>
        </a>
...
const renderGoodsList = list => {
    let goodsList = list.map(item => renderGoodsItem(item.title, item.price, item.picture));
